...
(function($) {
  ...
  function init() {
    $.ajax({
      ...
      complete:function(data, status, jqXHR){         //処理成功時
        ...
        //----- 製品型番マスタの保存 ------
        SeihinList(jsonResult.katabanList);
        ...
        if ("Retrieval" == jsonResult.mode) {
          ...
          $("#sekoatena").val(jsonResult.seko);                     //施工証明書宛名
          ...
          $("#mitumoriyukokigen").val(jsonResult.mitumoriyukokigen);      //見積有効期限
          $("#iraibi").text(jsonResult.iraibi);											//見積依頼日
          $("#kaitoubi").text(jsonResult.kaitoubi);										//見積回答日
          $("#kaitoubi-v").val(jsonResult.kaitoubi);                  //見積回答日(更新用)
          ...
          $("#knebiki").val(jsonResult.knebiki);							//値引金額
          $("#kritu").val(jsonResult.kritu);								  //値引率
  ...
  /*-------------------- 製品型番マスタ --------------------*/
  var oSeihinList;                          // 製品型番マスタ

  /*------------------------------------------*/
  /*----- 製品型番マスタオブジェクト生成 -----*/
  /*------------------------------------------*/
  var SeihinList = function(list) { //
	  oSeihinList = list;
  }
  /*------------------------------------------*/
  /*----- 製品型番マスタから取得します   -----*/
  /*------------------------------------------*/
  SeihinList.get = function(id) {
    if (oSeihinList != undefined) {
    	var matchData;
    	for (var key in oSeihinList) {
    		var data = oSeihinList[key];
    		if (data.kataban == id) {
    			matchData = data;
    			break;
    		}
  ...
  /*----- 型番変更再設定               -----*/
  /*------------------------------------------*/
  function seihinchangeset() {
    $(".seihinkataban").unbind('change');
    $(".seihinkataban").seihinchange();
  }
  /*------------------------------------------*/
  /*----- 日付イベント再設定         -----*/
  /*------------------------------------------*/
      ...
      complete:function(data, status, jqXHR){         //処理成功時
        ...
        if (optiondata != undefined) {
          ...
          for (var datakey in optionlist) {
            ...
            $("#omashine" + data.optioncd).val(data.mashine);
            $("#onyuka" + data.optioncd).val(data.nyuka);
            $("#okibou" + data.optioncd).val(data.kibou);
          }
        }
        datepickerset();
        switch (userkey.substring(0,2)) {
        case "01":
          $(".omashine").prop('disabled', true);
          $(".onyuka").prop('disabled', true);
          $(".okibou").prop('disabled', false);
          break;

        case "02":
          $(".omashine").prop('disabled', false);
          $(".onyuka").prop('disabled', false);
          $(".okibou").prop('disabled', true);
          break;
  ...
  /*----- 製品型番変更時                 -----*/
  /*------------------------------------------*/
  $.fn.seihinchange = function() {
    $(this).change(function() {
      seihincheck($(this), true);
    });
  }
  /*------------------------------------------*/
  /*----- 改造変更時                     -----*/
  ...
  /*----- 回答日更新クリック                                              -----*/
  /*------------------------------------------*/
  function anserupdate() {

    $("#kaitoubi-v").val(getNowYMD());
    $("#kaitoubi").text(getNowYMD());
    return;
  ...
  /*----- 製品型番チェック               -----*/
  /*------------------------------------------*/
  function seihincheck(seihin, bind) {
	  ...
	  //----- 製品型番の存在チェック -----
      targetkey = seihin.prop("id").replace("kataban", "");
      var katabandata = SeihinList.get(seihin.val());
      if (katabandata == undefined || katabandata == null) {
    	  Materialize.toast('入力された製品型番は存在しません。改造情報(' + (parseInt(targetkey) + 1) + '行目))', 4000, 'rounded');
      ...
      if (bind) {
	      //各項目に検索結果をバインドする
	      $("#kataban"     + targetkey).val(katabandata.kataban);
	      $("#kataban"     + targetkey).addClass('active');
	      $("#kBousyoku"   + targetkey).text(katabandata.bousyoku.toLocaleString()   + ' 円');
	      $("#kJubousyoku" + targetkey).text(katabandata.jubousyoku.toLocaleString() + ' 円');
	      $("#kTaiengai"   + targetkey).text(katabandata.taiengai.toLocaleString()   + ' 円');
	      $("#kTaijuengai" + targetkey).text(katabandata.taijuengai.toLocaleString() + ' 円');
	      $("#kJubousabi"  + targetkey).text(katabandata.jubousei.toLocaleString()   + ' 円');
	      $("#pBousyoku"   + targetkey).val('0');
	      $("#pJubousyoku" + targetkey).val('0');
	      $("#pTaiengai"   + targetkey).val('0');
	      $("#pTaijuengai" + targetkey).val('0');
	      $("#pJubousabi"  + targetkey).val('0');
	      var matchData = getkaizouinfo(targetkey);
	      matchData.kataban    = katabandata.kataban;
	      matchData.bousyoku   = katabandata.bousyoku;
	      matchData.jubousyoku = katabandata.jubousyoku;
	      matchData.taiengai   = katabandata.taiengai;
	      matchData.taijuengai = katabandata.taijuengai;
	      matchData.jubousei   = katabandata.jubousei;
	      updatekaizouinfo(targetkey,matchData);
      }
      return 1;
  ...
  function calculation() {
    ...
    for (var idx = 0; idx < licount; idx++) {
      ...
      var optiondata = getoptioninfo(oPreantCard.attr("gno"))[0];
      if (optiondata != undefined) {
        var optionlist = optiondata.option;
        for (var datakey in optionlist) {
          var data = optionlist[datakey];
    ...
    if (oKnebiki.val() != "" && parseInt(oKnebiki.val()) != 0) {
        knebiki = parseInt(oKnebiki.val().split(',').join('').trim());
        if (knebiki > total) {
  ...
  function errorCheck() {
    ...
    //改造情報の有効行チェック
    var kaizoResult = CardKaizoList.check();
    if (kaizoResult == -1) {
    	return false;
    }
    //特殊情報の有効行チェック
    var tokuResult = CardTokuList.check();
    //改造情報及び特殊改造に有効行が存在しない場合
    if ((kaizoResult + tokuResult) == 0) {
        Materialize.toast('改造情報および特殊改造に情報が入力されていません', 4000, 'rounded');
        return false;
    }
    //改造情報の必須チェック
  ...
  function estimateUpdate() {
    ...
    var json = {
        ...
        ,"kaitoubi":$("#kaitoubi-v").val()
        ,"mitumoriyukokigen":$("#mitumoriyukokigen").val()
            ...
            complete:function(data, status, jqXHR){         //処理成功時
              ...
              if (jsonResult.result == "success") {
                ...
                if (estimatemode == "Retrieval") {
                  window.location = pUrl + "/Retrieval/moveretrieval";
                }
                else {
  ...
  CardKaizoList.put = function(json) {
    ...
    if (oCardKaizoList != undefined) {
      for (var key in json) {
        ...
        var optionjson = {"key":data.gno, "option": datajson}
        optioninfo.push(optionjson);

        var oString = '<li class="card-panel" id="kaizo' + key + '" key="' + key + '" gno="' + data.gno + '">'
        + '<div class="row">'
        + '<div class="input-field col s12 l4">'
        + '<input id="kataban' + key + '"  name="kataban" type="text" placeholder="XXXXXXXXXXXXXXXXXXXX" value="' + data.kataban + '" class="seihinkataban">'
  ...
  CardKaizoList.add = function() {
    if (oCardKaizoList != undefined) {
      ...
      kaizouinfo.push(addJson);
      var oString = '<li class="card-panel" id="kaizo' + key + '" key="' + key + '" gno="' + key + '">'
      + '<div class="row">'
      + '<div class="input-field col s12 l4">'
      + '<input id="kataban' + key + '"  name="kataban" type="text" placeholder="XXXXXXXXXXXXXXXXXXXX" value="" class="seihinkataban">'
      + '<label for="kataban' + key + '" class="center-align active" id="kataban' + key + '-l" data="製品型番">製品型番</label>'
      + '</div>'
      + '<div class="col s6 l1 center">'
      + '<p>'
  ...
  CardKaizoList.bunno = function(idx) {
    ...
    if (oPreantCard != undefined) {
      ...
      + '<div class="row">'
      + '<div class="input-field col s6 l4">'
      + '<input id="bmashine' + gkey + '"  name="bmashine" type="text" placeholder="XXXXXXXXXX" value="' + $("#mashine" + idx).val() + '">'
      + '<label for="bmashine' + gkey + '" class="center-align active" id="bmashine' + gkey + '-l" data="マシンNo.">マシンNo.</label>'
      + '</div>'
      + '<div class="input-field col s6 l2">'
      + '<input id="bsuryo' + gkey + '"  name="bsuryo" type="text" class="right-text suryo" placeholder="99" value="0">'
      + '<label for="bsuryo' + gkey + '" class="center-align active" id="bsuryo' + gkey + '-l" data="数量">数量</label>'
      + '</div>'
      + '<div class="input-field col s12 l6">'
      + '<input id="bnyuka' + gkey + '"  name="bnyuka" type="text" value="' + $("#nyuka" + idx).val() + '" placeholder="yyyy/mm/dd" class="datepicker">'
      + '<label for="bnyuka' + gkey + '" class="center-align active" id="bnyuka' + gkey + '-l" data="入荷予定日">入荷予定日</label>'
      + '</div>'
      + '</div>'
      + '<div class="row">'
      + '<div class="input-field col s12 l6">'
      + '<input id="bkibou' + gkey + '"  name="bkibou" type="text" value="' + $("#kibou" + idx).val() + '" placeholder="yyyy/mm/dd" class="datepicker">'
      + '<label for="bkibou' + gkey + '" class="center-align active" id="bkibou' + gkey + '-l" data="入荷希望日">入荷希望日</label>'
      + '</div>'
      + '<div class="input-field col s12 l6">'
      + '<input id="bhikitori' + gkey + '"  name="bhikitori" type="text" value="' + $("#hikitori" + idx).val() + '" placeholder="yyyy/mm/dd" class="datepicker">'
      + '<label for="bhikitori' + gkey + '" class="center-align active" id="bhikitori' + gkey + '-l" data="引取希望日">引取希望日</label>'
      + '</div>'
      + '</div>'
      + '<div class="row">'
      + '<div class="input-field col s12 l6">'
      + '<input id="bhaise' + gkey + '"  name="bhaise" type="text" value="' + $("#haise" + idx).val() + '" placeholder="yyyy/mm/dd" class="datepicker">'
      + '<label for="bhaise' + gkey + '" class="center-align active" id="bhaise' + gkey + '-l" data="配セ戻入日">配セ戻入日</label>'
      + '</div>'
      + '<div class="input-field col s12 l6">'
      + '<input id="bkend' + gkey + '"  name="bkend" type="text" value="' + $("#kend" + idx).val() + '" placeholder="yyyy/mm/dd" class="datepicker">'
      + '<label for="bkend' + gkey + '" class="center-align active" id="bkend' + gkey + '-l" data="製造完了予定日">製造完了予定日</label>'
      + '</div>'
      + '</div>'
      + '</li>';
    ...
    if (oCardKaizoList != undefined) {
      ...
      for (var i=0; i < list.length; i++) {
        ...
        for (var i2=0; i2 < bunnolist.length; i2++) {
          var jsondata = {gno:i2
              , kataban:$("#kataban" + key).val()
              , bousyoku:$("#bousyoku" + key).prop("checked")
              , pBousyoku:$("#pBousyoku" + key).val()
              , Jubousyoku:$("#Jubousyoku" + key).prop("checked")
              , pJubousyoku:$("#pJubousyoku" + key).val()
              , taiengai:$("#taiengai" + key).prop("checked")
              , pTaiengai:$("#pTaiengai" + key).val()
              , taijuengai:$("#taijuengai" + key).prop("checked")
              , pTaijuengai:$("#pTaijuengai" + key).val()
              , jubousabi:$("#jubousabi" + key).prop("checked")
              , pJubousabi:$("#pJubousabi" + key).val()
              , mashine:$("#bmashine" + ((key * 100) + i2)).val()
              , suryo:$("#bsuryo" + ((key * 100) + i2)).val()
              , gsuryo:0
              , nyuka:$("#bnyuka" + ((key * 100) + i2)).val()
              , kibou:$("#bkibou" + ((key * 100) + i2)).val()
              , hikitori:$("#bhikitori" + ((key * 100) + i2)).val()
              , haise:$("#bhaise" + ((key * 100) + i2)).val()
              , kend:$("#bkend" + ((key * 100) + i2)).val()
          };
          gsuryo += parseInt($("#bsuryo" + ((key * 100) + i2)).val());
          bunnojson.push(jsondata);
        }

        gsuryo += parseInt($("#suryo" + key).val());
        var jsondata = {gno:i
            , kataban:$("#kataban" + key).val()
            , bousyoku:$("#bousyoku" + key).prop("checked")
            , pBousyoku:$("#pBousyoku" + key).val()
            ...
            , jubousabi:$("#jubousabi" + key).prop("checked")
            , pJubousabi:$("#pJubousabi" + key).val()
            , mashine:$("#mashine" + key).val()
            , suryo:$("#suryo" + key).val()
            , gsuryo:gsuryo
            , nyuka:$("#nyuka" + key).val()
            , kibou:$("#kibou" + key).val()
            , hikitori:$("#hikitori" + key).val()
            , haise:$("#haise" + key).val()
            , kend:$("#kend" + key).val()
            , bunnoinfo:bunnojson
        };
  ...
  /*----- 製品型番が有効な行をチェック   -----*/
  /*------------------------------------------*/
  //戻り値は【0：無効行】 【1：有効行】 【-1：型番未存在】
  CardKaizoList.check = function() {
	var result = 0;
    if (oCardKaizoList != undefined) {
      var list = oCardKaizoList.children("li");
      for (var i=0; i < list.length; i++) {
    	var seihin = $("#kataban" + $(list[i]).attr("key"));
    	var sresult = seihincheck(seihin, false);
    	if (result != -1 && sresult == -1) {
    		result = -1;
    	}
    	else {
    		result += sresult;
  ...
  CardTokuList.put = function(json) {
    ...
    if (oCardTokuList != undefined) {
      for (var key in json) {
        var data = json[key];
        var oString = '<li class="card-panel" id="tokusyu' + key + '" key="' + key + '">'
        + '<div class="row">'
        + '<div class="input-field col s12 l4">'
  ...
  CardTokuList.bunno = function(idx) {
    ...
    if (oPreantCard != undefined) {
      ...
      + '<div class="row">'
      + '<div class="input-field col s6 l4">'
      + '<input id="btmashine' + gkey + '"  name="btmashine" type="text" placeholder="XXXXXXXXXX" value="' + $("#tmashine" + idx).val() + '">'
      + '<label for="btmashine' + gkey + '" class="center-align active" id="btmashine' + gkey + '-l" data="マシンNo.">マシンNo.</label>'
      + '</div>'
      + '<div class="input-field col s6 l2">'
      + '<input id="btsuryo' + gkey + '"  name="btsuryo" type="text" class="right-text suryo" placeholder="99" value="0">'
      + '<label for="btsuryo' + gkey + '" class="center-align active" id="btsuryo' + gkey + '-l" data="数量">数量</label>'
      + '</div>'
      + '<div class="input-field col s12 l6">'
      + '<input id="btnyuka' + gkey + '"  name="btnyuka" type="text" value="' + $("#tnyuka" + idx).val() + '" placeholder="yyyy/mm/dd" class="datepicker">'
      + '<label for="btnyuka' + gkey + '" class="center-align active" id="btnyuka' + gkey + '-l" data="入荷予定日">入荷予定日</label>'
      + '</div>'
      + '</div>'
      + '<div class="row">'
      + '<div class="input-field col s12 l6">'
      + '<input id="btkibou' + gkey + '"  name="btkibou" type="text" value="' + $("#tkibou" + idx).val() + '" placeholder="yyyy/mm/dd" class="datepicker">'
      + '<label for="btkibou' + gkey + '" class="center-align active" id="btkibou' + gkey + '-l" data="入荷希望日">入荷希望日</label>'
      + '</div>'
      + '<div class="input-field col s12 l6">'
      + '<input id="bthikitori' + gkey + '"  name="bthikitori" type="text" value="' + $("#thikitori" + idx).val() + '" placeholder="yyyy/mm/dd" class="datepicker">'
      + '<label for="bthikitori' + gkey + '" class="center-align active" id="bthikitori' + gkey + '-l" data="引取希望日">引取希望日</label>'
      + '</div>'
      + '</div>'
      + '<div class="row">'
      + '<div class="input-field col s12 l6">'
      + '<input id="bthaise' + gkey + '"  name="bthaise" type="text" value="' + $("#thaise" + idx).val() + '" placeholder="yyyy/mm/dd" class="datepicker">'
      + '<label for="bthaise' + gkey + '" class="center-align active" id="bthaise' + gkey + '-l" data="配セ戻入日">配セ戻入日</label>'
      + '</div>'
      + '<div class="input-field col s12 l6">'
      + '<input id="btkend' + gkey + '"  name="btkend" type="text" value="' + $("#tkend" + idx).val() + '" placeholder="yyyy/mm/dd" class="datepicker">'
      + '<label for="btkend' + gkey + '" class="center-align active" id="btkend' + gkey + '-l" data="製造完了予定日">製造完了予定日</label>'
      + '</div>'
      + '</div>'
      + '</li>';
    ...
    if (oCardTokuList != undefined) {
      ...
      for (var i=0; i < list.length; i++) {
        ...
        for (var i2=0; i2 < bunnolist.length; i2++) {
          var jsondata = {
              gno:i2
             , tkataban:$("#tkataban" + key).val()
             , tkaizou:$("#tkaizou" + key).val()
             , tkingaku:$("#tkingaku" + key).val()
             , ptkingaku:$("#ptkingaku" + key).val()
             , tmashine:$("#btmashine" + ((key * 100) + i2)).val()
             , tsuryo:$("#btsuryo" + ((key * 100) + i2)).val()
             , tgsuryo:0
             , tnyuka:$("#btnyuka" + ((key * 100) + i2)).val()
             , tkibou:$("#btkibou" + ((key * 100) + i2)).val()
             , thikitori:$("#bthikitori" + ((key * 100) + i2)).val()
             , thaise:$("#bthaise" + ((key * 100) + i2)).val()
             , tkend:$("#btkend" + ((key * 100) + i2)).val()
          };
          gsuryo += parseInt($("#btsuryo" + ((key * 100) + i2)).val());
          bunnojson.push(jsondata);
        }
        gsuryo += parseInt($("#tsuryo" + key).val());
        var jsondata = {
            gno:i
           , tkataban:$("#tkataban" + key).val()
           , tkaizou:$("#tkaizou" + key).val()
           , tkingaku:$("#tkingaku" + key).val()
           , ptkingaku:$("#ptkingaku" + key).val()
           , tmashine:$("#tmashine" + key).val()
           , tsuryo:$("#tsuryo" + key).val()
           , tgsuryo:gsuryo
           , tnyuka:$("#tnyuka" + key).val()
           , tkibou:$("#tkibou" + key).val()
           , thikitori:$("#thikitori" + key).val()
           , thaise:$("#thaise" + key).val()
           , tkend:$("#tkend" + key).val()
           , bunnoinfo:bunnojson
        };
        json.push(jsondata);
  ...
  /*----- 要素の値をJSON形式で取得します -----*/
  /*------------------------------------------*/
  CardTokuList.check = function() {
	var result = 0;
    if (oCardTokuList != undefined) {
      var list = oCardTokuList.children("li");
      for (var i=0; i < list.length; i++) {
      	var seihin = $("#tkataban" + i);
    	if (seihin.val() == "") { //製品型番が未入力の場合は無効行
    	}
    	else {
    		result++;
    	}
  ...
  CardOptionList.put = function(json) {
    ...
    if (oCardOptionList != undefined) {
      for (var key in json) {
        ...
        + '<div class="row">'
        + '<div class="input-field col s6 l4">'
        + '<input id="omashine' + key + '"  name="omashine" type="text" value="" class="omashine">'
        + '<label for="omashine' + key + '" class="center-align active">マシンNo.</label>'
        + '</div>'
        + '<div class="input-field col s12 l4">'
        + '<input id="onyuka' + key + '"  name="onyuka" type="text" value="" class="datepicker onyuka">'
        + '<label for="onyuka' + key + '" class="center-align active">入荷予定日</label>'
        + '</div>'
        + '<div class="input-field col s12 l4">'
        + '<input id="okibou' + key + '"  name="okibou" type="text" value="" class="datepicker okibou">'
        + '<label for="okibou' + key + '" class="center-align active">入荷希望日</label>'
        + '</div>'
        + '</div>'
        + '</li>';
  ...
  function estimateControl(kengen, stts) {
    ...
    ContorolInfo.add({"info":0 , "item":"anserupdate", control:{"0000":{"disabled":true , "required":false}, "0110":{"disabled":false, "required":false}, "0120":{"disabled":false, "required":false}}, type:"btn"});
    ContorolInfo.add({"info":0 , "item":"mitumoriyukokigen", control:{"0000":{"disabled":true, "required":false}, "0110":{"disabled":false, "required":false}, "0120":{"disabled":false, "required":false}}, type:"date"});
    ...
    ContorolInfo.add({"info":0 , "item":"addr1"      , control:{"0000":{"disabled":false, "required":false}, "0110":{"disabled":false, "required":true}, "0210":{"disabled":false, "required":true}, "9910":{"disabled":false, "required":true}, "0120":{"disabled":false, "required":true}, "0220":{"disabled":false, "required":true}, "9920":{"disabled":false, "required":true}, "0320":{"disabled":false, "required":true}, "0230":{"disabled":true, "required":false}, "9930":{"disabled":true, "required":false}, "0130":{"disabled":true, "required":false}, "0330":{"disabled":true, "required":false}, "0225":{"disabled":true, "required":false}, "0125":{"disabled":true, "required":false}, "0140":{"disabled":true, "required":false}, "0240":{"disabled":true, "required":false}, "9940":{"disabled":true, "required":false}, "0325":{"disabled":true, "required":false}, "9925":{"disabled":true, "required":false}}, type:"wide"});
    ContorolInfo.add({"info":0 , "item":"addr2"      , control:{"0000":{"disabled":false, "required":false}, "0230":{"disabled":true, "required":false}, "9930":{"disabled":true, "required":false}, "0130":{"disabled":true, "required":false}, "0330":{"disabled":true, "required":false}, "0225":{"disabled":true, "required":false}, "0125":{"disabled":true, "required":false}, "0140":{"disabled":true, "required":false}, "0240":{"disabled":true, "required":false}, "9940":{"disabled":true, "required":false}, "0325":{"disabled":true, "required":false}, "9925":{"disabled":true, "required":false}}, type:"wide"});
    ContorolInfo.add({"info":0 , "item":"tel"        , control:{"0000":{"disabled":false, "required":false}, "0230":{"disabled":true, "required":false}, "9930":{"disabled":true, "required":false}, "0130":{"disabled":true, "required":false}, "0330":{"disabled":true, "required":false}, "0225":{"disabled":true, "required":false}, "0125":{"disabled":true, "required":false}, "0140":{"disabled":true, "required":false}, "0240":{"disabled":true, "required":false}, "9940":{"disabled":true, "required":false}, "0325":{"disabled":true, "required":false}, "9925":{"disabled":true, "required":false}}, type:"tel"});
    ContorolInfo.add({"info":0 , "item":"fax"        , control:{"0000":{"disabled":false, "required":false}, "0230":{"disabled":true, "required":false}, "9930":{"disabled":true, "required":false}, "0130":{"disabled":true, "required":false}, "0330":{"disabled":true, "required":false}, "0225":{"disabled":true, "required":false}, "0125":{"disabled":true, "required":false}, "0140":{"disabled":true, "required":false}, "0240":{"disabled":true, "required":false}, "9940":{"disabled":true, "required":false}, "0325":{"disabled":true, "required":false}, "9925":{"disabled":true, "required":false}}, type:"tel"});
    ...
    ContorolInfo.add({"info":1 , "item":"mashine"    , control:{"0000":{"disabled":true , "required":false}, "0220":{"disabled":false, "required":false}, "9920":{"disabled":true, "required":false}, "0320":{"disabled":false, "required":false}, "0225":{"disabled":true, "required":false}, "0125":{"disabled":true, "required":false}, "0140":{"disabled":true, "required":false}, "0240":{"disabled":true, "required":false}, "9940":{"disabled":true, "required":false}, "0325":{"disabled":true, "required":false}, "9925":{"disabled":true, "required":false}, "9930":{"disabled":true, "required":false}, "9940":{"disabled":true, "required":false}}, type:"alnum"});
    ContorolInfo.add({"info":1 , "item":"suryo"      , control:{"0000":{"disabled":false, "required":false} , "0110":{"disabled":false, "required":true} , "0210":{"disabled":false, "required":true}, "9910":{"disabled":false, "required":true}, "0120":{"disabled":false, "required":true}, "0220":{"disabled":false, "required":false}, "9920":{"disabled":true, "required":false}, "0320":{"disabled":false, "required":true}, "0225":{"disabled":true, "required":false}, "0125":{"disabled":true, "required":false}, "0140":{"disabled":true, "required":false}, "0240":{"disabled":true, "required":false}, "9940":{"disabled":true, "required":false}, "0325":{"disabled":true, "required":false}, "9925":{"disabled":true, "required":false}, "9930":{"disabled":true, "required":false}, "9940":{"disabled":true, "required":false}}, type:"num"});
    ContorolInfo.add({"info":1 , "item":"nyuka"      , control:{"0000":{"disabled":true , "required":false}, "0220":{"disabled":false, "required":false}, "9920":{"disabled":true, "required":false}, "0320":{"disabled":false, "required":true}, "0225":{"disabled":true, "required":false}, "0125":{"disabled":true, "required":false}, "0140":{"disabled":true, "required":false}, "0325":{"disabled":true, "required":false}, "9930":{"disabled":true, "required":false}, "9940":{"disabled":true, "required":false}}, type:"date"});
    ...
    //----- 改造情報(分納) -----
    ContorolInfo.add({"info":1 , "item":"bmashine"    , control:{"0000":{"disabled":true , "required":false}, "0220":{"disabled":false, "required":false}, "9920":{"disabled":true, "required":false}, "0320":{"disabled":false, "required":false}, "0225":{"disabled":true, "required":false}, "0125":{"disabled":true, "required":false}, "0140":{"disabled":true, "required":false}, "0240":{"disabled":true, "required":false}, "9940":{"disabled":true, "required":false}, "0325":{"disabled":true, "required":false}, "9925":{"disabled":true, "required":false}, "9930":{"disabled":true, "required":false}, "9940":{"disabled":true, "required":false}}, type:"alnum"});
    ContorolInfo.add({"info":1 , "item":"bsuryo"      , control:{"0000":{"disabled":false, "required":false} , "0110":{"disabled":false, "required":true} , "0210":{"disabled":false, "required":true}, "9910":{"disabled":false, "required":true}, "0120":{"disabled":false, "required":true}, "0220":{"disabled":false, "required":false}, "9920":{"disabled":true, "required":false}, "0320":{"disabled":false, "required":true}, "0225":{"disabled":true, "required":false}, "0125":{"disabled":true, "required":false}, "0140":{"disabled":true, "required":false}, "0240":{"disabled":true, "required":false}, "9940":{"disabled":true, "required":false}, "0325":{"disabled":true, "required":false}, "9925":{"disabled":true, "required":false}, "9930":{"disabled":true, "required":false}, "9940":{"disabled":true, "required":false}}, type:"num"});
    ContorolInfo.add({"info":1 , "item":"bnyuka"      , control:{"0000":{"disabled":true , "required":false}, "0220":{"disabled":false, "required":false}, "9920":{"disabled":true, "required":false}, "0320":{"disabled":false, "required":true}, "0225":{"disabled":true, "required":false}, "0125":{"disabled":true, "required":false}, "0140":{"disabled":true, "required":false}, "0325":{"disabled":true, "required":false}, "9930":{"disabled":true, "required":false}, "9940":{"disabled":true, "required":false}}, type:"date"});
    ...
    ContorolInfo.add({"info":2 , "item":"tmashine"   , control:{"0000":{"disabled":true , "required":false}, "0220":{"disabled":false, "required":false}, "9920":{"disabled":true, "required":false}, "0320":{"disabled":false, "required":false}, "0225":{"disabled":true, "required":false}, "0125":{"disabled":true, "required":false}, "0140":{"disabled":true, "required":false}, "0240":{"disabled":true, "required":false}, "9940":{"disabled":true, "required":false}, "0325":{"disabled":true, "required":false}, "9925":{"disabled":true, "required":false}, "9930":{"disabled":true, "required":false}, "9940":{"disabled":true, "required":false}}, type:"alnum"});
    ContorolInfo.add({"info":2 , "item":"tsuryo"     , control:{"0000":{"disabled":false, "required":false} , "0110":{"disabled":false, "required":true}, "0120":{"disabled":false, "required":true}, "0210":{"disabled":false, "required":true}, "9910":{"disabled":false, "required":true}, "0220":{"disabled":false, "required":false}, "9920":{"disabled":true, "required":false}, "0320":{"disabled":false, "required":true}, "0225":{"disabled":true, "required":false}, "0125":{"disabled":true, "required":false}, "0140":{"disabled":true, "required":false}, "0240":{"disabled":true, "required":false}, "9940":{"disabled":true, "required":false}, "0325":{"disabled":true, "required":false}, "9925":{"disabled":true, "required":false}, "9930":{"disabled":true, "required":false}, "9940":{"disabled":true, "required":false}}, type:"num"});
    ContorolInfo.add({"info":2 , "item":"tnyuka"     , control:{"0000":{"disabled":true , "required":false}, "0220":{"disabled":false, "required":false}, "9920":{"disabled":true, "required":false}, "0320":{"disabled":false, "required":true}, "0225":{"disabled":true, "required":false}, "0125":{"disabled":true, "required":false}, "0140":{"disabled":true, "required":false}, "0240":{"disabled":true, "required":false}, "9940":{"disabled":true, "required":false}, "0325":{"disabled":true, "required":false}, "9925":{"disabled":true, "required":false}, "9930":{"disabled":true, "required":false}, "9940":{"disabled":true, "required":false}}, type:"date"});
    ...
    ContorolInfo.add({"info":2 , "item":"tkend"      , control:{"0000":{"disabled":true , "required":false}, "0220":{"disabled":true, "required":false}, "9920":{"disabled":true, "required":false}, "0320":{"disabled":true, "required":false}, "0225":{"disabled":true, "required":false}, "0120":{"disabled":false, "required":true}, "0125":{"disabled":false, "required":true}, "0130":{"disabled":false, "required":true}, "0140":{"disabled":true, "required":false}, "0240":{"disabled":true, "required":false}, "9940":{"disabled":true, "required":false}, "0325":{"disabled":true, "required":false}, "9925":{"disabled":true, "required":false}, "9930":{"disabled":true, "required":false}, "9940":{"disabled":true, "required":false}}, type:"date"});
    //----- 特殊改造(分納) -----
    ContorolInfo.add({"info":2 , "item":"btmashine"   , control:{"0000":{"disabled":true , "required":false}, "0220":{"disabled":false, "required":false}, "9920":{"disabled":true, "required":false}, "0320":{"disabled":false, "required":false}, "0225":{"disabled":true, "required":false}, "0125":{"disabled":true, "required":false}, "0140":{"disabled":true, "required":false}, "0240":{"disabled":true, "required":false}, "9940":{"disabled":true, "required":false}, "0325":{"disabled":true, "required":false}, "9925":{"disabled":true, "required":false}, "9930":{"disabled":true, "required":false}, "9940":{"disabled":true, "required":false}}, type:"alnum"});
    ContorolInfo.add({"info":2 , "item":"btsuryo"     , control:{"0000":{"disabled":false, "required":false} , "0110":{"disabled":false, "required":true}, "0120":{"disabled":false, "required":true}, "0210":{"disabled":false, "required":true}, "9910":{"disabled":false, "required":true}, "0220":{"disabled":false, "required":false}, "9920":{"disabled":true, "required":false}, "0320":{"disabled":false, "required":true}, "0225":{"disabled":true, "required":false}, "0125":{"disabled":true, "required":false}, "0140":{"disabled":true, "required":false}, "0240":{"disabled":true, "required":false}, "9940":{"disabled":true, "required":false}, "0325":{"disabled":true, "required":false}, "9925":{"disabled":true, "required":false}, "9930":{"disabled":true, "required":false}, "9940":{"disabled":true, "required":false}}, type:"num"});
    ContorolInfo.add({"info":2 , "item":"btnyuka"     , control:{"0000":{"disabled":true , "required":false}, "0220":{"disabled":false, "required":false}, "9920":{"disabled":true, "required":false}, "0320":{"disabled":false, "required":true}, "0225":{"disabled":true, "required":false}, "0125":{"disabled":true, "required":false}, "0140":{"disabled":true, "required":false}, "0240":{"disabled":true, "required":false}, "9940":{"disabled":true, "required":false}, "0325":{"disabled":true, "required":false}, "9925":{"disabled":true, "required":false}, "9930":{"disabled":true, "required":false}, "9940":{"disabled":true, "required":false}}, type:"date"});
    ...
    ContorolInfo.add({"info":4 , "item":"estdeletebtn", control:{"0000":{"disabled":true , "required":false}, "0110":{"disabled":false, "required":false}, "0210":{"disabled":false, "required":false}, "9910":{"disabled":false, "required":false}, "0120":{"disabled":false, "required":false}, "0125":{"disabled":false, "required":false}, "0130":{"disabled":false, "required":false}, "0220":{"disabled":false, "required":false}, "9920":{"disabled":false, "required":false}, "0325":{"disabled":false, "required":false}, "9925":{"disabled":false, "required":false}}, type:"btn"});
    ...
    ContorolInfo.add({"info":4 , "item":"kaizobtn"    , control:{"0000":{"disabled":true , "required":false},"0120":{"disabled":false , "required":false},"0220":{"disabled":false , "required":false},"0320":{"disabled":false , "required":false},"0125":{"disabled":false , "required":false},"0225":{"disabled":false , "required":false},"0325":{"disabled":false , "required":false},"0130":{"disabled":false , "required":false},"0230":{"disabled":false , "required":false},"0330":{"disabled":false , "required":false},"0140":{"disabled":false , "required":false},"0240":{"disabled":false , "required":false},"0340":{"disabled":false , "required":false}}, type:"btn"});
